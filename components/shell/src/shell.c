...
esp_err_t shell_init(const shell_cfg_t *cfg)
{
    ...
    if (cfg != NULL) {
        ...
        local_cfg.enable_bsp_commands = cfg->enable_bsp_commands;
    }
    ...
    ret = shell_i2c_register_commands();
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "register i2c commands failed: %s", esp_err_to_name(ret));
        (void)destroy_repl();
        return ret;
    }

    if (local_cfg.enable_bsp_commands) {
        ret = shell_register_bsp_commands();
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "register bsp commands failed: %s", esp_err_to_name(ret));
            (void)destroy_repl();
            return ret;
        }
    }
